...
> 当前为提交整理候选版，不是已完成所有验收的正式发布。源码与示例随仓库提供，六项官方模型依赖已核对并固定版本；本项目适配器的百度网盘链接已提供，发布方确认长期有效，独立下载核验尚待完成，固定运行源码包也有待补齐，见 [大文件下载清单](assets/DOWNLOADS.md)。在补齐材料前，新用户不能仅靠本仓库复现全部真实模型推理。已有完整环境的用户可配置现有路径运行。这里只说明第一阶段，不包含第二阶段统一基座或变化掩膜能力。
...
适配器下载：[百度网盘——挑战杯-698832](https://pan.baidu.com/s/1bKJZ7LO6APt-LjidIA7wyA?pwd=xhst)，**提取码：`xhst`**。

- 版本：`phase1-20260903-rc1`；文件名：`challenge-cup-phase1-adapter_phase1-20260903-rc1.zip`，139,921,887字节（约133.44 MiB）。
- 分享有效期：**长期有效（发布方确认）**；是否需要百度账号登录及独立下载验证：待核验。
- 在分享目录内找到该ZIP和同名 `.zip.sha256` 文件，下载后先核验哈希，再解压到模型目录的父目录；ZIP已有一层 `general_adapter/`，不要重复嵌套。预期SHA256及完整说明见 [下载清单](assets/DOWNLOADS.md)。
