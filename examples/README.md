...
1. 在服务器仓库根目录运行 `python3 scripts/client.py samples` 查看八个请求。
...
| xlrs_reasoning | 4096×4096原始JPEG已附 | 储罐、厂房和道路对应的工业区域推理 | 历史独立评测原始输出 `C`，数据集参考答案 `C`；不是统一网关新请求 |
| mme_color | 2098×2098原始PNG已附 | 下方矩形场地中集装箱的颜色 | 历史独立评测原始输出 `B`，数据集参考答案 `B`；不是统一网关新请求 |
...
`catalog.json`中的图像路径是仓库相对路径；`client.py`在服务器运行时转换成绝对路径后再提交，API本身仍只接收服务器绝对路径。`reference_outputs/`保留原有网关记录；`showcase/`保存主页三例的原输入、参考答案、模型原始输出、图像哈希和历史证据。路径已去除内部账号信息，答案和耗时未改写。

主页三例是特意选择的正确案例，不是随机测试样本，不能用来报告总体准确率。“原输出”指数据集参考答案，不是未经记录的基线输出。两例独立评测的完整提示词由保留的原评测器函数重建；原始题目、选项和预测直接来自历史记录，JSON对来源作了区分。

历史小图冷启动总耗时17.234666秒、热启动1.131054秒；大图细节请求总耗时127.972183秒。这些是单条记录，不是均值、P95或硬件性能承诺；也不是本次重新运行所得。请以自己的任务返回为准。

图像与官方数据集的归属见 `assets/DATASETS.md`。本次附带3张小原图及2张主页展示大图，共5张；没有上传整个数据集，也没有缩小大图来替代历史输入。旧的 `03553_Toronto.png` 仍需另外从官方数据源下载，不影响自带的 `mme_color` 案例。
